...
const config: Config = {
  ...
  // moduleNameMapper: {
  //   '@/(.*)': ['<rootDir>/$1'],
  //   // minhoteca_layer_aws: '<rootDir>/minhoteca_layer_aws/src/index',
  // },
  moduleNameMapper: {
    '^(\\.{1,2}/.*)\\.js$': '$1',
  },
  ...
  // },
  transform: {
    // '^.+\\.[tj]sx?$' para processar js/ts/jsx/tsx com `ts-jest`
    // '\\.m?[tj]sx?$' para processar js/ts/jsx/tsx/mjs/mts com `ts-jest`
    '^.+\\.m?[tj]sx?$': [
      'ts-jest',
      {
        useESM: true,
